...
parser.add_argument('--num_avg', type=int, default=5,
                    help='')
...
# Model and optimizer
AAGNN_model = AAGNN_multi_avg(feature_size=features.shape[1],
...
AAGNN_model.eval()
polluted_train_emb = AAGNN_model(features, adj_dense, degree_mat, args.num_avg).detach().cpu().numpy()
...
if not os.path.exists(name):
    ...
    for epoch in range(args.epochs):
        ...
        AAGNN_model.train()
        optimizer.zero_grad()
        output = AAGNN_model(features, adj_dense, degree_mat, args.num_avg)
        ...
        # evaluate in val set
        AAGNN_model.eval()
        output_val = AAGNN_model(features, adj_dense, degree_mat, args.num_avg)
...
AAGNN_model.eval()
test_pred_emb = AAGNN_model(features, adj_dense, degree_mat, args.num_avg)
