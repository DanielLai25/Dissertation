...
parser.add_argument('--auto_2nd_layer', type=int, default=2048,
                    help='')
parser.add_argument('--auto_dropout', type=float, default=0,
                    help='')

parser.add_argument('--AAGNN_ori_epochs', type=int, default= 150,
                    help='Number of epoch to train.')
...
auto_adj_para_name = "auto_adj_parameter" + str(args.dataset) + "_" + str(args.auto_epochs) + "_" + str(args.auto_2nd_layer) + ".pt"
...
autoencoder_adj_model = Autoencoder(input_size = adj_dense.shape[1],
                                first_layer_size = args.auto_1st_layer,
                                second_layer_size = args.auto_2nd_layer,
                                dropout_ratio = args.auto_dropout)

autoencoder_adj_optimizer = optim.Adam(autoencoder_adj_model.parameters(), lr=args.auto_lr,\
                                   betas=(args.auto_beta1, args.auto_beta2),eps=1e-8)
autoencoder_adj_model.to(device)
adj_dense_noise = add_noise(adj_dense).to(device)
adj_dense = adj_dense.to(device)

if not os.path.exists(auto_adj_para_name):
    print("no auto adj parameter exists")

    early_stopping = EarlyStopping(patience=args.patient, verbose=True, path=auto_adj_para_name)
    for epoch in range(args.auto_epochs):
        t = time.time()
        autoencoder_adj_model.train()
        autoencoder_adj_optimizer.zero_grad()
        # features_noisy = autoencoder_model.add_noise(features).to(device)
        _, output = autoencoder_adj_model(adj_dense)
        emb_loss_train = nn.KLDivLoss()(output, adj_dense)
        emb_loss_train.backward()
        autoencoder_optimizer.step()
    ...
    torch.save(autoencoder_adj_model.state_dict(), auto_adj_para_name)
    print("auto parameter saved")

else:
    print("auto parameter loaded")
    autoencoder_adj_model.load_state_dict(torch.load(auto_adj_para_name))

adj_emb, adj_dec = autoencoder_adj_model(adj_dense_noise)
