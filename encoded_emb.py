...
features_center = torch.mean(features, 0)

sps_outlier_list = []
for i in range(features.shape[0]):
    sps_outlier_list.append(anomaly_score(features[i, :], features_center).item())
...
"""AAGNN"""
AAGNN_ori_model = AAGNN(feature_size=emb.shape[1],
...
emb = emb.cpu()
polluted_train_emb = AAGNN_ori_model(emb, adj_dense, degree_mat).detach().cpu().numpy()
...
if not os.path.exists(AAGNN_ori_para_name):
    ...
    for epoch in range(args.AAGNN_ori_epochs):
        ...
        output = AAGNN_ori_model(emb, adj_dense, degree_mat)
        ...
        AAGNN_ori_model.eval()
        output_val = AAGNN_ori_model(emb, adj_dense, degree_mat)
...
emb = emb.cpu()
final_ori_emb = AAGNN_ori_model(emb, adj_dense, degree_mat)
